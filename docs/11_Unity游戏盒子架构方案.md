...
| **D-14** | 预算线 | **✅ 已决:首包 ≤60MB / 新玩法 ≤8MB / 内存 ≤200MB / 冷启动 ≤2.5s(§11)** | 原 10 文档误编为 D-8(与 HybridCLR 版本撞号),2026-08-21 D 表一次对齐顺延为 D-14;两文档编号自此一致 |
| **D-15** | UI 动画方案 | **✅ 已决:自研补间(UniTask 扩展 `BoxTween`,EaseOutBack 等曲线自实现),不引入 DOTween/PrimeTween** | 数独品类动效需求轻量(弹窗弹入/按钮反馈/数字脉冲/转场,自研 100~150 行覆盖 90%);第三方动画库在 IL2CPP + 解释执行下有泛型/反射踩坑位且增依赖面。2026-08-21 用户拍板,§3.5 UIKit 依赖清单不变 |
...
| 项 | 状态 | 说明 / 剩余动作 |
|---|---|---|
| 本文入库(D-10) | ✅ **已完成** | `.gitignore` 已移除 `docs/11_…` 与 `docs/07_…` 的忽略规则;`_private/`(调研原文与备份)继续忽略。**docs/10_… 已入库(2026-08-21 拍板)**:10 文档已重写为权威执行手册,不再是调研原文 |
