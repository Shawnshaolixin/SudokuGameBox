...
> **2026-09-16 补记 · 本包已重建**：v17 此前未上传任何轨道，故重建后 versionCode
> **仍是 17**，未消耗新版本号。重建（commit `890c790`）引入两处变化：
> ① 启动画面关闭 Unity logo（`m_ShowUnitySplashLogo: 0`），保留 app 图标；
> ② `versionName` 由 `1.0` 改为 `1.0.0`。
> 前者是品牌清理、后者纯展示串，均非玩家功能，**英文上架文案保持上面的兜底句不变**。
