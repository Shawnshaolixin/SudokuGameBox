...
| 预算 | 首包 ≤60MB / 新玩法 ≤8MB / 内存 ≤200MB / 冷启动 ≤2.5s | D-8 |
| 权限 | SudokuCore 永久留 AOT，禁止进热更程序集 | 红线 |

## 3. 决策清单 D-1 ~ D-13（已定版，变更需用户拍板）

| # | 决策 | 状态 |
|---|---|---|
| D-1 | 热更框架 HybridCLR 社区版 + Addressables Remote Catalog 远程下发；v1.0 纯 AOT 自包含上架，v1.1 才启用 | ✅ |
| D-2 | v1.0 纯 AOT 自包含（无热更依赖） | ✅ |
| D-3 | Trait 框架全 AOT，热更侧只放 Trait 子类 | ✅ |
| D-5 | 单货币 box.coins | ✅ |
| D-6 | 配置四分管（数据/规则/表现/运行期） | ✅ |
| D-7 | AES-GCM 单文件分区存档 + v0→v1 迁移器 | ✅ |
| D-8 | 预算线（首包/新玩法/内存/冷启动） | ✅ |
...
> 注：D-4/D-9/D-10 空缺号为讨论中未定案项（随 Phase 推进补充）。
