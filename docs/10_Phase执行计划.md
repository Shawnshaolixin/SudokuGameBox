...
| M3 商业化+引导+热更验证 | ✅ 3.1~3.4 代码/构建侧全部验收（2026-09-05）；末环=真机冒烟（用户今晨装机 v1.0 交付 APK）+ 发布形态拍板 | 激励视频三点位 → 插屏频控配置化（AOT 一次改造）→ 引导 3 步通用件 → v1.1 热更构建验证 + 发布形态决策 |
...
**M3.4 热更构建验证证据（2026-09-05，详见 19 文档 §10）**：名单 4 处同步加 `Box.HotUpdate.WaterSort`（Phase9HybridCLRSetup / HotUpdateService / link.xml / GenerateContent module_overrides v1.1.0）；
v1.1 链实测（PrepareV11 → 内部 GenerateContent → BuildV11Apk 免签名中间验证）：三个热更程序集均过 FilterHotFixAssemblies 过滤、RemoteContent + BuiltinHotUpdate 双份拷贝、libil2cpp 39 处 HybridCLR 符号 + RuntimeApi 在案、收尾还原 enable=0 + 符号移除；
Phase9Publish.BuildAll 全量远程构建成功（catalog 与 ServerData 最新 hotupdate_local bundle 同 hash）；
**v1.0 交付 APK**（`Rovilo-debug-v13-20260905-0826.apk` 85.8MB，7 bundle 随包含 watersort + hotupdate_builtin 兜底）——真机冒烟在用户装机后验收；
AAB/release 被 BOX_KEYSTORE_PASS 阻塞，决策后走 17 号文档指南。
