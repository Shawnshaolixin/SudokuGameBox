...
### 4.1 数据安全表单（Play Console）

> **2026-09-16 补正（勿删）**：本节原先只列了"广告标识符、崩溃数据、应用互动"，
> **漏了「大致位置」**——这正是生产轨道发布被打回的原因。依据 Google 官方 AdMob
> 数据披露表，AdMob 会收集 IP 地址并"可能用于估算设备的大致位置"，
> **这与应用是否申请定位权限无关**。隐私政策原文写「不请求位置权限」与表单口径
> 直接打架（两边都漏尚可辩解，一边勾一边否认必被打回）。**两份口径必须始终一致。**

**① 收集的数据类型**（与 [privacy-policy.html](./privacy-policy.html) §1 表格一一对应）

⚠️ **「收集」和「分享」是两个独立的勾选框，五类都必须勾「分享」**——数据是传给 Google（AdMob / Firebase）的，
对开发者而言 Google 就是第三方。2026-09-16 导出审核发现「大致位置」当时只勾了「收集」、漏了「分享」，
而其余四类都勾了，属同一链路却两套口径。

| Data Safety 类别 | 子项 | 收集 | 分享 | 来源 |
|---|---|---|---|---|
| 设备 ID 或其他 ID | 设备或其他 ID | ✅ | ✅ | AdMob（AAID、应用集 ID） |
| 位置 | **大致位置**（依据 IP 推算） | ✅ | ✅ | AdMob / Firebase |
| 应用活动 | **仅「应用互动」** | ✅ | ✅ | Firebase Analytics / AdMob |
| 应用信息和性能 | **崩溃日志** + **诊断信息** | ✅ | ✅ | Crashlytics / AdMob |
| 财务信息 | **交易记录** | ✅ | ✅ | Unity IAP 收到的 `Purchase` + `iap_purchase_complete` 埋点 |
| 个人信息 / 消息 / 照片和视频 / 音频 / 文件与文档 / 通讯录 / 日历 / 网页浏览 / 健康与健身 | — | ❌ | ❌ | 无 |
| 应用活动 → 应用内搜索记录 / 已安装的应用 / 其他由用户生成的内容 / 其他操作 | — | ❌ | ❌ | 无 |
| 应用信息和性能 → 其他应用性能数据 | — | ❌ | ❌ | 若 Console 仍报错再加勾：**多报的惩罚远小于漏报** |

**收集与分享的用途**：广告/分析四类（设备 ID / 大致位置 / 应用互动 / 崩溃日志+诊断）统一为
**分析** + **广告或营销**；**交易记录**为 **应用功能** + **分析**（本地记录以兑现去广告权益 + 埋点漏斗），
**不含**广告或营销。不勾「账号管理」（本应用无账号系统）。
「用户控制」统一填**必须收集**——商品一旦购买，App 必须记录才能兑现去广告权益。

**为什么「交易记录」要勾**（判据是「你自己有没有处理」，**不是**「Billing 给了收据」）

Google 对 Purchase history 的定义是 *"Information about purchases or transactions that a user has made."*
它把两件事分得很开：

- **支付服务自己收的金融信息**（信用卡号等）：App 没访问就**不用申报**；
- **购买历史**：只要**你自己**从支付服务收到购买相关数据并加以使用，就**要申报**。

Rovilo 的实现（[UnityIapService.cs](../GameBox/Assets/Services/Iap/UnityIapService.cs)）落在「要申报」那侧：

| 实现事实 | 位置 | 对应申报 |
|---|---|---|
| 收到 `PurchaseEventArgs.purchasedProduct`，读 `definition.id` / `hasReceipt` | `ProcessPurchase` L151-161 | 收集 |
| 写入本地存档 D-7 分区 `box.commerce` | `CompletePurchase` L208-211 | 收集（本地） |
| 商品 ID 作为 `product` 参数发往 Firebase Analytics | L217-219 | **分享**（给 Google） |
| 信用卡等支付金融信息 | 从未接触 | 不申报（与「交易记录」是两回事） |

> `product.receipt`（收据原文）**没有被外传**，代码只用了 `hasReceipt` 这个布尔量——
> 但这不影响结论：判据是「是否收到并使用了购买相关信息」，商品 ID 已经够了。

**② 共享 / 安全 / 删除**

- 加密传输：**是**（全部经 HTTPS/TLS）；
- 可否删除：**是** → 删除请求途径 = 邮件 `slx97065227@gmail.com`（已写入隐私政策 §4），本地存档卸载即删。
  - ⚠️ **2026-09-16 导出显示实际填的是「否」**，与已上线的隐私政策（写明可邮件请求删除）冲突——
    与「大致位置」是同一类错误：**一侧承认、另一侧否认**。
    建议改填「是」并把 URL 指向政策页：声明有删除途径比声明没有更安全；
    若坚持「否」，则须删掉政策里的邮件那句以保持口径一致。
- 账号相关：账号创建方法 = **我的应用不允许用户创建账号**；无外部账号登录；账号删除 URL 留空（无账号，N/A）。

> ⚠️ `com.google.android.gms.permission.AD_ID` 由 AdMob SDK 的 AAR 合并进 manifest
> （工程自身 manifest **没有**声明），**表单仍须如实勾选**。
> ⚠️ 判断依据是 **SDK 实际行为**，不是工程有没有申请权限。工程零定位权限 ≠ 表单不勾「大致位置」。
> 对照 [05_合规_发布_测试.md](./05_合规_发布_测试.md) 的完整清单逐项过。

### 4.2 内容分级（IARC 问卷）

- **内容**适合全年龄（无用户生成内容 / 社交 / 抽卡，无暴力血腥）→ IARC 据此给出低龄分级（如商店展示 "Rated for 3+"），这一项**不要**因为受众是 13+ 就往上调；
- 有广告但不含"参考赌博"类激励（激励视频看广告获提示属常规变现，如实回答即可）。
- ⚠️ **别把「内容分级」和「目标受众」混为一谈**：本节答的是"内容适合几岁"，而"应用面向几岁"在 Play Console「目标受众和内容」单独勾选，本作只勾 **13+**（见 05 号文档 §1）。二者可以不一致，且**必须**区分——2026-09-10 曾因此误勾 6-8/9-12 触发家庭政策风险。
